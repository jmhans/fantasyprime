...
class PHP_CRUD_API {
	...
	protected function createFile($input) {
		if (!$input) return false;
		$input = (array)$input;
		//$keys = implode(',',str_split(str_repeat('!', count($input))));
		//$values = implode(',',str_split(str_repeat('?', count($input))));
	...
	protected function createFiles($inputs) {
		if (!$inputs) return false;
		$ids = array();
		foreach ($inputs as $input) {
			$result = $this->createObject($input);
			if ($result===null) {
				return null;
			}
			$ids[] = $result;
	...
	protected function createFileCommand($parameters) {
		extract($parameters);
		if (!$inputs || !$inputs[0]) $this->exitWith404('input');
		if (count($inputs)>1) return $this->createFiles($inputs);
		return $this->createFile($inputs[0]);
	}
	...
	public function executeCommand() {
		...
		if (!$this->settings['request']) {
			...
			switch($parameters['action']){
				...
				case 'create': $output = $this->createFileCommand($parameters); break;
