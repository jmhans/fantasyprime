	...
	protected function saveFile($parameters) {
	   ...
	   if ($inputs != null)
	   {
		 ...
		 if ($flName != null) {
			...
			if (!$originalDataObj->games) {
				$originalDataObj->games = [];
			}
			var_dump($originalDataObj->games);
			$key = $this->findGameKey($originalDataObj->games, $inputs->gamePk);
			echo("Key is:" . $key);
			var_dump($originalDataObj->games);
			echo("Inputs Marker:");
			var_dump($inputs);
			if ($key > -1){
				array_splice($originalDataObj->games, $key, 1, array($inputs));
//				array_replace($originalDataObj->games, array($key => $inputs));
			} else {
				array_push($originalDataObj->games, $inputs);
			}
	...
	protected function findGameKey($gameArr, $gamePk) {
		echo("marker_line");
		$result = [];
		foreach($gameArr as $key => $obj) {
			$result[$obj->gamePk] = $key;
		}
		if (array_key_exists($gamePk, $result)) {
			return $result[$gamePk];
		} else {
			return -1;
		}
